...
// never in the mobile menu. The thumbnail is a nav-free capture of the page
// (public/images/page-previews, regenerated by
// scripts/capture-page-previews.py). aria-hidden keeps the description out
// of the link's spoken name; the page's own <meta> description covers that.
// `withImage` is false until the pointer first enters the nav, so the twelve
// thumbnails are only fetched by people who can actually hover.
function NavTooltip({ href, withImage }: { href: string; withImage: boolean }) {
  return (
    <p className={tooltipClass} aria-hidden="true">
      {withImage && (
        <Image
          ...
          height={750}
          sizes="400px"
          className={`${styles['nav-tooltip-image']} margin-bottom-8px`}
        />
      )}
          ...
          >
            {visibleItems.map((item, i) => (
              ...
              >
                ...
                <NavTooltip href={item.href} withImage={previewsArmed} />
              </Link>
            ))}
